...
import { getAllUsersInVacancyService, getVacancyByIdService, hideUserInVacancyService } from "../../../api/apiVacancies";
import { useSearchParams } from "react-router-dom";
import { getPhaseService, updateLlamadaPhaseService } from "../../../api/apiPhase";
import Swal from "sweetalert2";
...
const VacancieDetail = () => {
    ...
    const fetchForMatch = async (page, newPerPage) => {
      try {
        ...
        //const response = await axios.get(endpointURL);
         ...
         const response = await getAllUsersInVacancyService(idVacancy, page, newPerPage);
        const datasUsers = response?.item?.docs || [];
        //console.log("datos de los usuarios:..", datasUsers[0]);

        setDataByUserCandidate(datasUsers[0].applicants||[]);
        setTotalRows(datasUsers[0].applicants?.length||0);
        setLoading(false);
      } catch (error) {
        //console.log(error);
      }
    ...
    const queryVacancy = async () => {
      try {
        ...
        const response = await getVacancyByIdService(idVacancy);
        const dataVacancy = response?.infoVacancy;
        //console.log("datos vacante", dataVacancy);
        setDataInfoVacancy(dataVacancy);
      } catch (error) {
        console.log("Error al recuperar los datos de la vacante:..", error);
      }
    };
    useEffect(() => {
      queryVacancy();
    ...
    const queryPhase = async () => {
      ...
      for (const phase of phases) {
        ...
        try {
          const responseDataPhase = await getPhaseService(phase);
  
          const result = responseDataPhase.infoPhase?.vacancies;
        ...
        if (result.isConfirmed) {
          ...
          if (hideCandidate) {
            ...
            try {
              ...
              const responseHideUser = await hideUserInVacancyService({
                idVacancy,
                idCandidate: id,
                emailUser: hideCandidate.email,
                });
              console.log("responseBack HideUser:..", responseHideUser);
            } catch (error) {
        ...
        if (result.isConfirmed) {
          ...
          if (dataRepet) {
            ...
            try {
              ...
              const responseUpdatePhase = await updateLlamadaPhaseService(idVacancy,selectUser._id);
              //console.log("responseBackend (candidato Agregado):..", response);
              if (responseUpdatePhase) {
                queryPhase();
              }
            } catch (error) {
      ...
      <>
        <div className="row m-3">
          ...
          <div className="col">
            ...
            {dataByUserCandidate?.length > 0 && dataInfoVacancy && (
              <MyTable
        ...
        <div className="d-flex w-100 justify-content-end p-4">
          <Link to={dataByUserCandidate?.length===0?'#':`/recPanel?v=${idVacancy}&title=${dataInfoVacancy.title}&company=${dataInfoVacancy.companyName}`}>
