...
import { getVacancyByIdService, closeVacancyService, hideUserInVacancyService } from '../../../api/apiVacancies';
import { getPhaseService, updatePanelService } from '../../../api/apiPhase';
import { findUserService } from '../../../api/api';
...
const RecPanel = () => {
    ...
    const cargarDatos = async () => {
      ...
      const dataVacancy = await getVacancyByIdService(idVacancy);
          //console.log('DataVacancy (rejecteds):..',dataVacancy.data.infoVacancy.rejecteds);
      const rejecteds=dataVacancy?.infoVacancy?.rejecteds;
      for (const phase of phases) {
        ...
        try {
          //const result = await axios.get(endpointPhase);
            const result = await getPhaseService(phase);
            //console.log("resultPhase:...", result);
          const dataVacancies = result.infoPhase.vacancies;
          ...
          if (listadoApplicants) {
            for (let i = 0; i < listadoApplicants?.length; i++) {
              ...
              const response = await findUserService(listadoApplicants[i]);
              //console.log('response DataApplicants:...',response);
              if (response) {
                const applicant = {...response};
      ...
      //console.log("tempDataApplicants:...", tempDataApplicants);
    ...
    const closeVacancy=async()=>{
      ...
      try {
        ...
        const responseCloseVacancy= await closeVacancyService(dataBody);
        //console.log('response Close Vacancy:..',responseCloseVacancy);
      } catch (error) {
        console.log(error);
      }
  ...
  try {
            ...
            const responseUpdatePanel = await updatePanelService(dataBody);
            //console.log("Updating Panel de Reclutamiento:...", response);
            if (responseUpdatePanel) {
              Swal.fire({
                title: "Guardando",
          ...
          if (hideCandidate) {
            ...
            try {
              ...
              /* const responseBack = await axios.post(
                ...
                const responseBack = await hideUserInVacancyService({
                    idVacancy,
                    idCandidate: id,
